...
function DashboardContent() {
    ...
    const loadData = () => {
      ...
      setTimeout(() => {
        ...
        if (savedChatrooms.length === 0) {
          ...
          storage.setChatrooms(defaultChatrooms);
        } else {
  ...
  const updateChatroomTitle = (chatroomId, newTitle) => {
    const updatedChatrooms = chatrooms.map((room) =>
      room.id === chatroomId ? { ...room, title: newTitle } : room
    );
    setChatrooms(updatedChatrooms);
    storage.setChatrooms(updatedChatrooms);
  };
  ...
  const togglePinChatroom = (chatroomId) => {
    ...
    toast.success(
      chatroom?.pinned ? "Conversation unpinned" : "Conversation pinned"
    );
  ...
  const deleteChatroom = (chatroomId) => {
    ...
    setChatrooms(updatedChatrooms);
    storage.setChatrooms(updatedChatrooms);
    localStorage.removeItem(`gemini-messages_${chatroomId}`);
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-[#0F0F0F]">
      ...
      <div
        className={`${
          isMobile && sidebarOpen ? "fixed" : "relative"
        } flex-shrink-0 transition-all duration-300 ease-in-out ${
          sidebarOpen ? "w-80" : "w-16"
        } ${
          isMobile && sidebarOpen ? "h-full left-0 top-0" : ""
        } border-r border-gray-200 dark:border-gray-800 flex flex-col bg-white dark:bg-[#1A1A1A] overflow-hidden z-50`}
      ...
      <div className="flex-1 flex flex-col">
        {selectedChatroom ? (
          <Chatroom
            ...
            chatroomTitle={
              chatrooms.find((room) => room.id === selectedChatroom)?.title ||
              "Chatroom"
            }
          ...
          <div className="flex-1 flex items-center justify-center bg-gray-50 dark:bg-[#0F0F0F]">
            <div className="text-center max-w-md">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-8 leading-relaxed">
                I'm Google's most capable AI model, built to be helpful,
                harmless, and honest. I can assist with writing, analysis, math,
